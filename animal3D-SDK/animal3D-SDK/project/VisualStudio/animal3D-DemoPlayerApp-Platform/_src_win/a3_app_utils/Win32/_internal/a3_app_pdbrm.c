				...
				for (i = 0; i < nProcInfo; ++i)
				{
					...
					if (procHandle)
					{
						...
						if (!NT_SUCCESS(status) || status == STATUS_INFO_LENGTH_MISMATCH)
						{
							...
							handleInfo = (PSYSTEM_HANDLE_INFORMATION)realloc(handleInfo, returnLen);
							...
							for (j = 0; j < handleInfo->HandleCount; ++j)
							{
								...
								if (handle.ProcessId == pid)
								{
									...
									if (dupHandle && NT_SUCCESS(status))
									{
										...
										{
											objectTypeInfo = (POBJECT_TYPE_INFORMATION)realloc(objectTypeInfo, returnLen);
											status = NtQueryObject(dupHandle, ObjectTypeInformation, objectTypeInfo, returnLen, &returnLen);
										}

										// looking for object of 'File' type
										if (NT_SUCCESS(status) && wcsstr(objectTypeInfo->Name.Buffer, L"File") != 0)
										{
											// query object can hang if this flag is set
											if (handle.GrantedAccess != ACCESS_GRANTED_FLAG)
											{
												...
												if (!NT_SUCCESS(status) || status == STATUS_OBJECT_PATH_INVALID)
												{
													objectNameInfo = (PUNICODE_STRING)realloc(objectNameInfo, returnLen);
													status = NtQueryObject(dupHandle, ObjectNameInformation, objectNameInfo, returnLen, &returnLen);
												}
												CloseHandle(dupHandle);

												// looking for pdb
												if (NT_SUCCESS(status) && wcsstr(objectNameInfo->Buffer, pdbNamew) != 0)
												{
